...
},
{
	...
	cost: 175,
	onDamageModifier: function(damageCalc){
		damageCalc.STAB = 2;
	},
},
{
	...
	cost: 125,
	onFaint: function(context, player){
		if(player.lastAttacker != null && player != player.lastAttacker){
			...
			if(!attacker.has(API.EFFECTS.NONVOLATILE.FAINTED) &&
				...
				API.sendMessage(context.channel, attacker.mention()+" took damage due to "+player.mention()+"'s aftermath!");
				attacker.damage(context.channel, attacker.getMaxHP()/4);
			}
			delete attacker;
		}
	},
},
{
	...
	onDamageModifier: function(damageCalc, attacker, move, defender){
		if(move.getType() == API.TYPES.NORMAL){
			damageCalc.power *= 1.3;
			damageCalc.changeMoveType(API.TYPES.FLYING);
		}
	},
	cost: 200,
},
{
	...
	onBegin: function(context, player){
	//TODO this glitches up with more than one cloud-nine/airlock
		if(context.battle != null){
			context.battle.set(API.EFFECTS.BATTLE.CLOUD_NINE, -1);
			API.sendMessage(context.channel, player.mention()+"'s Air Lock canceled all weather!");
		}
	},
	onEnd: function(context, player){
		if(context.battle != null){
			context.battle.clear(API.EFFECTS.BATTLE.CLOUD_NINE);
			API.sendMessage(context.channel, player.mention()+"'s Air Lock faded away...");
		}
	},
	cost: 150,
},
